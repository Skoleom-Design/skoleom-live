...
export default function BoostCheckoutPage() {
  ...
  const { client_secret, id } = router.query;
  const [success, setSuccess] = useState(false);
  const [confirmError, setConfirmError] = useState('');

  async function handlePaymentSuccess() {
    try {
      if (typeof id === 'string') await api.post(`/payments/boost/${id}/confirm`, {});
      setSuccess(true);
    } catch (err) {
      setConfirmError(err instanceof ApiError ? err.message : "Le paiement a réussi mais l'activation a échoué — contacte le support.");
    }
  }
  ...
  return (
    <>
      <Head>
        <title>Paiement du boost — skoleomLive</title>
      </Head>
        ...
        <div className="w-full max-w-sm">
          {success ? (
            <div className="text-center space-y-4">
              ...
              <h1 className="text-xl font-bold text-white">Boost activé</h1>
              <p className="text-white/45 text-sm">Ta campagne est active dès maintenant.</p>
              <Link
                href="/profile/me"
            ...
            <>
              <h1 className="text-xl font-bold text-white mb-6 text-center">Payer le boost</h1>
              {confirmError && (
